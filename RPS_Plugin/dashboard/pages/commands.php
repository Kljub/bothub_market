...
$commands = [
    'rps-plugin:rps'      => ['cmd' => '/rps',      'label' => 'RPS (solo)', 'desc' => 'Schere-Stein-Papier gegen den Bot, Einsatz optional', 'defaultPerms' => []],
    'rps-plugin:rps-duel' => ['cmd' => '/rps-duel',  'label' => 'RPS-Duell',  'desc' => 'Schere-Stein-Papier gegen einen anderen User, Einsatz optional', 'defaultPerms' => []],
];
...
if ($botId > 0) {
    ...
    $stmt = $db->prepare('SELECT module_key, enabled, settings FROM bot_module_states WHERE bot_id = ? AND module_key LIKE "rps-plugin:%"');
    $stmt->execute([$botId]);
...
<div class="bh-card bh-card-lg">
    <div class="bh-card-header">
        <h2>RPS-Commands (<?= count($commands) ?>)</h2>
    </div>
    <p class="bh-text-muted bh-text-sm" style="padding:12px 16px 0;margin:0;">
        Der Einsatz ist bei beiden Commands optional — ohne Einsatz-Option läuft das Spiel komplett ohne Economy-Zugriff.
    </p>
    <div id="rps-commands">
        <?php foreach ($commands as $mk => $mod):
            ...
            $panelId = 'perm-rps-' . str_replace(':', '-', $mk);
        ?>
