...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $botId > 0) {
    if (($_SESSION['csrf_token'] ?? '') !== ($_POST['csrf'] ?? '')) {
        ...
        $success = bh_plugin_t($pk, 'config.settings_saved');
        ...
        $success = bh_plugin_t($pk, 'config.untrack_saved');
    }
}
...
<div class="bh-card bh-card-lg" style="margin-bottom:20px;">
    ...
    <form method="post" style="padding:14px 16px;display:flex;gap:10px;flex-wrap:wrap;align-items:end;">
        ...
        <div class="bh-form-group">
            <label class="bh-label"><?= $e(bh_plugin_t($pk, 'config.server_label')) ?></label>
            <select id="ani-guild-select" class="bh-input">
                <option value=""><?= $e(bh_plugin_t($pk, 'config.pick_server')) ?></option>
        ...
        <div class="bh-form-group">
            <label class="bh-label"><?= $e(bh_plugin_t($pk, 'config.target_channel_label')) ?></label>
            <select id="ani-channel-select" class="bh-input">
                <option value=""><?= $e(bh_plugin_t($pk, 'config.pick_server_first')) ?></option>
                <?php if ($curChannel): ?>
                <option value="<?= $e($curChannel) ?>" selected><?= $e(bh_plugin_t($pk, 'config.current_prefix', ['channel' => $curChannel])) ?></option>
                <?php endif; ?>
            </select>
        </div>
        <button type="submit" class="bh-btn bh-btn-primary"><?= $e(bh_plugin_t($pk, 'config.save_btn')) ?></button>
    </form>
</div>

<div class="bh-card bh-card-lg" style="margin-bottom:20px;">
    <div class="bh-card-header">
        <h2><?= $e(bh_plugin_t($pk, 'config.tracked_title', ['count' => count($tracked)])) ?></h2>
    </div>
    <p class="bh-text-muted bh-text-sm" style="padding:0 16px;margin:8px 0;"><?= bh_plugin_t($pk, 'config.add_hint', ['cmd' => '<code style="background:var(--bg-secondary);padding:1px 5px;border-radius:3px">/anisearch-track</code>']) ?></p>
    <?php if (!$tracked): ?>
    <p class="bh-text-muted bh-text-sm" style="padding:14px 16px;"><?= $e(bh_plugin_t($pk, 'config.no_tracked')) ?></p>
    <?php else: ?>
    <div style="display:flex;flex-direction:column;">
        ...
        <div style="display:flex;align-items:center;gap:10px;padding:10px 16px;border-bottom:1px solid var(--border);">
            <div style="flex:1;min-width:0;">
                ...
                <span class="bh-text-muted bh-text-sm"><?= $t['last_known_episode'] !== null ? $e(bh_plugin_t($pk, 'config.next_episode', ['episode' => (int)$t['last_known_episode']])) : '' ?></span>
            </div>
            <form method="post" onsubmit="return confirm(<?= $e(json_encode(bh_plugin_t($pk, 'config.confirm_untrack', ['title' => $t['title']]), JSON_UNESCAPED_UNICODE)) ?>);">
                <input type="hidden" name="csrf" value="<?= $e($csrf) ?>">
...
<div class="bh-card">
    <div class="bh-card-title">🎮 <?= $e(__('cmd.title')) ?></div>
...
(function () {
    ...
    const I18N = {
        loading: <?= json_encode(bh_plugin_t($pk, 'config.js_loading')) ?>,
        pickChannel: <?= json_encode(bh_plugin_t($pk, 'config.js_pick_channel')) ?>,
        loadError: <?= json_encode(bh_plugin_t($pk, 'config.js_load_error')) ?>
    };
    ...
    if (guildSelect) {
        guildSelect.addEventListener('change', async function () {
            ...
            try {
                ...
                channelSelect.innerHTML = '<option value="">' + I18N.pickChannel + '</option>';
