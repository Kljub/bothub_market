...
$commands = [
    'minesweeper-plugin:minesweeper' => ['cmd' => '/minesweeper', 'label' => 'Minesweeper', 'desc' => 'Minenfeld-Wette starten — 🔥-Button zahlt aus', 'defaultPerms' => []],
];
...
if ($botId > 0) {
    ...
    $stmt = $db->prepare('SELECT module_key, enabled, settings FROM bot_module_states WHERE bot_id = ? AND module_key LIKE "minesweeper-plugin:%"');
    $stmt->execute([$botId]);
...
<div class="bh-card bh-card-lg">
    <div class="bh-card-header">
        <h2>Minesweeper-Commands (<?= count($commands) ?>)</h2>
    </div>
    <div id="minesweeper-commands">
        <?php foreach ($commands as $mk => $mod):
            ...
            $panelId = 'perm-minesweeper-' . str_replace(':', '-', $mk);
        ?>
