...
$commands = [
    'criminal-plugin:steal' => ['cmd' => '/steal', 'label' => 'Steal', 'desc' => 'Guthaben von einem anderen User klauen', 'defaultPerms' => []],
];
...
if ($botId > 0) {
    ...
    $stmt = $db->prepare('SELECT module_key, enabled, settings FROM bot_module_states WHERE bot_id = ? AND module_key LIKE "criminal-plugin:%"');
    $stmt->execute([$botId]);
...
<div class="bh-card bh-card-lg">
    <div class="bh-card-header">
        <h2>Criminal-Commands (<?= count($commands) ?>)</h2>
    </div>
    <div id="criminal-commands">
        <?php foreach ($commands as $mk => $mod):
            ...
            $panelId = 'perm-criminal-' . str_replace(':', '-', $mk);
        ?>
