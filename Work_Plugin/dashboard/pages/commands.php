...
if ($botId > 0) {
    ...
    $stmt = $db->prepare('SELECT module_key, enabled, settings FROM bot_module_states WHERE bot_id = ? AND module_key LIKE "work-plugin:%"');
    $stmt->execute([$botId]);
...
<div class="bh-card bh-card-lg">
    ...
    <div id="work-commands">
        <?php foreach ($commands as $mk => $mod):
            ...
            $panelId = 'perm-work-' . str_replace(':', '-', $mk);
        ?>
